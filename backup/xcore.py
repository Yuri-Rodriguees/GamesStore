...
from PyQt5.QtCore import Qt, QUrl, QSize, QMimeData, QPoint, QPropertyAnimation, QEasingCurve, QTimer, pyqtSignal, QThread, QRect, QRectF, pyqtProperty, QEventLoop, pyqtSignal
from PyQt5.QtGui import QFont, QIcon, QDesktopServices, QDragEnterEvent, QDropEvent, QColor, QPalette, QPen, QLinearGradient, QPainter, QPainterPath, QPixmap
from PyQt5.QtWidgets import (QApplication, QWidget, QPushButton, QLabel, QVBoxLayout, 
                            QHBoxLayout, QLineEdit, QFrame, QStackedWidget, QListWidget, 
                            QFileDialog, QMessageBox, QGraphicsBlurEffect, QGraphicsColorizeEffect, QGraphicsOpacityEffect, QProgressBar, QGraphicsDropShadowEffect)

class DownloadThreadJogo(QThread):
    update_signal = pyqtSignal(str, int)  # Mensagem, progresso
    finished_signal = pyqtSignal(str)     # Caminho do arquivo
    error_signal = pyqtSignal(str)        # Mensagem de erro
    ...
    def run(self):
        try:
            self.update_signal.emit("Obtendo URL de download...", 10)
            self.download_url = self.get_download_url(self.game_id)
            
            if not self.download_url:
                raise Exception("Falha ao obter URL de download")
            
            self.update_signal.emit("Processando link de download...", 30)
            final_url = self.ouo_bypass(self.download_url)
            
            if not final_url:
                raise Exception("Falha ao processar link")
            
            self.update_signal.emit("Preparando download...", 50)
            file_path = self.download_file(final_url)
    ...
    def download_file(self, url):
        safe_name = "".join(c for c in self.game_name if c.isalnum() or c in (' ', '.', '_')).rstrip()
        file_path = os.path.join(self.temp_dir, f"{safe_name}.zip")
    ...
    def handle_download_progress(self, bytes_received, bytes_total):
        if bytes_total > 0:
            ...
            self.update_signal.emit(f"Baixando... {bytes_received/1024/1024:.1f}MB de {bytes_total/1024/1024:.1f}MB", 
                                  50 + int(progress * 0.5))
    ...
    def handle_download_finished(self, reply, file_path):
        if reply.error() == QNetworkReply.NoError:
            ...
            raise Exception(f"Erro no download: {reply.errorString()}")
    ...
    def RecaptchaV3(self):
        try:
            ...
            if not match:
                raise Exception("Falha ao analisar a URL do anchor")
            
            url_base += match.group(1) + '/'
            params = match.group(2)
            
            res = client.get(url_base + 'anchor', params=params, impersonate="chrome110")
            token_match = re.search(r'"recaptcha-token" value="(.*?)"', res.text)
            if not token_match:
                raise Exception("Falha ao encontrar token do recaptcha")
            ...
            params_dict.setdefault('v', '')
            params_dict.setdefault('k', '')
            params_dict.setdefault('co', '')
            ...
            answer_match = re.search(r'"rresp","(.*?)"', res.text)
            if not answer_match:
                raise Exception("Falha ao obter resposta do recaptcha")
            
            return answer_match.group(1)
        except Exception as e:
            print(f"Erro no RecaptchaV3: {e}")
            return "token_falso"

    def ouo_bypass(self, url):
        try:
            client = cffi_requests.Session()
            client.headers.update({
                'authority': 'ouo.io',
            ...
            for _ in range(2):
                ...
                if not bs4.form:
                    raise Exception("Nenhum formulário encontrado na página OUO")
            ...
            print(f"Erro no bypass OUO: {e}")
            return None

    def get_download_url(self, game_id):
        try:
            headers = {
            ...
            if response.status_code != 200:
                raise Exception(f"Falha na requisição à API com status {response.status_code}")
            
            data = response.json()
            
            if data.get("status") != "success" or not data.get("downloadUrl"):
                raise Exception("Resposta inválida da API: " + json.dumps(data))
    ...
    def run(self):
        try:
            ...
            if self._is_cancelled:
                return
            data = response.json()
            results = [{"name": item["name"], "id": item["id"]} for item in data.get("items", [])]
            self.finished.emit(results)
        except Exception as e:
            if not self._is_cancelled:
                self.error.emit(str(e))  # Apenas emite a mensagem de erro
...
class GameApp(QWidget):
    ...
    def start_download(self, game_id, game_name):
        ...
        self.download_thread = DownloadThreadJogo(game_id, game_name)
        self.download_thread.update_signal.connect(self.handle_download_update)
    ...
    def handle_download_update(self, message, progress):
        self.progress_label.setText(message)
    ...
    def handle_download_complete(self, file_path):
        ...
        if not game_name or not game_id:
            QMessageBox.critical(self, "Erro", "Nome ou ID do jogo não disponível.")
            return

        new_file_name = f"{game_name} ({game_id}).zip"
        new_file_path = os.path.join(os.path.dirname(file_path), new_file_name)

        try:
            os.rename(file_path, new_file_path)
        except Exception as e:
            QMessageBox.critical(self, "Erro ao Renomear", f"Falha ao renomear o arquivo:\n{str(e)}")
            return

        extract_folder = os.path.join(os.path.dirname(new_file_path), f"{game_name} ({game_id})")

        try:
            with zipfile.ZipFile(new_file_path, 'r') as zip_ref:
                zip_ref.extractall(extract_folder)
        except Exception as e:
            QMessageBox.critical(self, "Erro ao Extrair", f"Falha ao extrair o zip:\n{str(e)}")
            return
        ...
        try:
            ...
            self.register_installation2(game_name, game_id, steam_path, moved_files)

            QMessageBox.information(
                self, 'Instalação Completa',
                f'{game_name} ({game_id}) foi instalado com sucesso!',
                QMessageBox.Ok
            )
            ...
            QMessageBox.critical(self, "Erro", f"Falha no processamento dos arquivos:\n{str(e)}")
    ...
    def handle_download_error(self, error):
        ...
        QMessageBox.critical(
            ...
            f'Ocorreu um erro ao baixar o jogo:\n\n{error}',
            QMessageBox.Ok
        )
