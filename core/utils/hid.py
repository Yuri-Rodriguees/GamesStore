...
def ensure_hid_dll():
    """Garante que xinput1_4.dll esteja na pasta da Steam"""
    try:
        ...
        # Localizar xinput1_4.dll na pasta config do app
        ...
        source_dll = os.path.join(base_path, "config", "xinput1_4.dll")
        if not os.path.exists(source_dll):
            # Tentar na pasta assets/config como fallback
            source_dll = os.path.join(base_path, "assets", "config", "xinput1_4.dll")
            
        if not os.path.exists(source_dll):
            log_message(f"[HID] DLL original não encontrada em: {source_dll}")
            return

        dest_dll = os.path.join(steam_path, "xinput1_4.dll")
        ...
        if should_copy:
            try:
                ...
                log_message(f"[HID] xinput1_4.dll copiada para: {dest_dll}")
            except Exception as e:
                log_message(f"[HID] Erro ao copiar xinput1_4.dll: {e}")
        else:
            log_message("[HID] xinput1_4.dll já existe e parece atualizada")
